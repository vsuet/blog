...
fetch("https://jsonplaceholder.typicode.com/users")
    ...
    .then(usersListArr => usersListArr.forEach(user => {
        usersList.innerHTML += `
        <div class="user">
            <p><a href="index2.html?id=${user.id}" class="link-user">${user.username}</a></p> 
        </div>               
        `
        users.push(user)
    }))
...
function viewPhotos () {
    ...
    while (i < 7) {
        images.innerHTML += `
            <img alt="" src="${pictures[i].url}" class="image">
        `
        i++
    }
}
...
postsList.onclick = (click) => {
    ...
    if (target.classList.contains('post-title')) {
        ...
        postUser.innerHTML = `(Пользователь <a href="index2.html?id=${users[userid].id}">${users[userid].username}</a>)`
